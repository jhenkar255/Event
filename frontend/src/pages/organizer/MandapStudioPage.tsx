...
import { Layers, ArrowLeft, Calendar, Sparkles, Building, ChevronRight } from 'lucide-react';

export const MandapStudioPage: React.FC = () => {
  ...
  useEffect(() => {
    api
      ...
      .catch((err) => console.error('Failed to load events for Mandap Studio:', err))
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex items-center space-x-4 relative z-10">
          ...
          <div>
            ...
            <h1 className="font-heading text-xl sm:text-2xl font-bold text-utsav-gold flex items-center space-x-2 mt-1">
              <Layers className="w-6 h-6 text-utsav-saffron" />
              <span>2D Royal Mandap & Stage Architecture Studio</span>
            </h1>
            <p className="text-xs text-utsav-ivory/80">
              Drag-and-drop traditional floral mandaps, havan kunds, varmala stages, and royal entry arches.
            </p>
          </div>
        </div>
        ...
        <div className="relative z-10 flex items-center space-x-2 bg-utsav-maroon-950/80 p-2 rounded-2xl border border-utsav-gold/40">
          ...
          <div className="flex flex-col">
            ...
            {loading ? (
              ...
              <span className="text-xs text-utsav-gold font-bold">Standard Demo Blueprint</span>
            ) : (
              <select
      ...
      {currentEvent && (
        <div className="flex items-center justify-between px-4 py-2.5 rounded-2xl bg-utsav-ivory dark:bg-utsav-maroon-900/60 border border-utsav-gold/30 text-xs">
          <div className="flex items-center space-x-3 text-utsav-brown dark:text-utsav-ivory">
            ...
            <span>•</span>
            <span className="text-gray-500">Tradition: {currentEvent.culturalTradition || 'Vedic'}</span>
            <span>•</span>
            <span className="text-gray-500">City: {currentEvent.location?.city || 'Jaipur'}</span>
          </div>
          <Link
      ...
      {/* Interactive Mandap Designer Canvas */}
