...
export const seedDatabase = async (): Promise<void> => {
  try {
    ...
    await Entertainment.insertMany([
      {
    ...
    const flagshipEvent = await Event.create({
    ...
    for (const g of sampleGuestNames) {
      ...
      const qrToken = QRService.generateSignedToken(flagshipEvent._id.toString(), guestDoc._id.toString());
    ...
    const venueBooking = await Booking.create({
      eventId: flagshipEvent._id,
    ...
    const decorBooking = await Booking.create({
      eventId: flagshipEvent._id,
      userId: standardUser._id,
      itemType: 'DECORATION',
    ...
    const cateringBooking = await Booking.create({
      eventId: flagshipEvent._id,
      userId: standardUser._id,
      itemType: 'CATERING',
      itemId: 'cat-shahi-01',
    ...
    const entertainmentBooking = await Booking.create({
      eventId: flagshipEvent._id,
      userId: standardUser._id,
