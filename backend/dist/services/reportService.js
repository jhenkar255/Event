...
class ReportService {
    ...
    static exportUsersCsv(users) {
        ...
        const records = csvStringifier.stringifyRecords(users.map((u) => ({
            _id: u._id?.toString(),
            name: u.name,
            email: u.email,
            phone: u.phone || 'N/A',
    ...
    static exportEventsCsv(events) {
        ...
        const header = csvStringifier.getHeaderString();
        const records = csvStringifier.stringifyRecords(events.map((e) => ({
            eventId: e.eventId,
            name: e.name,
            type: e.type,
    ...
    static exportPaymentsCsv(payments) {
        ...
        const header = csvStringifier.getHeaderString();
        const records = csvStringifier.stringifyRecords(payments.map((p) => ({
     ...
     */
    static exportBookingsCsv(bookings) {
        const csvStringifier = (0, csv_writer_1.createObjectCsvStringifier)({
            header: [
        ...
        const header = csvStringifier.getHeaderString();
        const records = csvStringifier.stringifyRecords(bookings.map((b) => ({
